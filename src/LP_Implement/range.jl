...
function vatpvg_ranges(net::MetNet, δvatp, vatp_idx, δvg, vg_idx; 
        vatp_margin::Float64 = 0.0,
        vg_margin::Float64 = 0.0,
    )
    # ranges
    vatpL, vatpU = fva(net, vatp_idx)
    vatp_range = vrange(vatpL - vatp_margin, vatpU + vg_margin, δvatp)
    ...
    for (vatpi, vatp) in enumerate(vatp_range)
        ...
        vg_ranges[vatpi] = vrange(vgL - vg_margin, vgU + vg_margin, δvg)
    end
...
function ivatpvg_ranges(net_pool::Vector{MetNet}, δvatp, vatp_idx, δvg, vg_idx; 
        vatp_margin::Float64 = 0.0,
        vg_margin::Float64 = 0.0,
    )
    ...
    @threads for (vatpi, vatp) in i_vatp_range
        ...
        vgL, vgU = fixxing(() -> fva(net, vg_idx), net, vatp_idx, vatp)
        vg_ranges[vatpi] = vrange(vgL - vg_margin, vgU + vg_margin, δvg)
    end
