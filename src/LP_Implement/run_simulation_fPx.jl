        ...
        ittime = @elapsed begin
            ...
            if 0.0 <= γ < 1.0 
                ...
                for (vatpi, vatp) in i_vatp_range
                    ...
                    new_vatp = discretize(γ * vatp, M.δvatp, vatp_range; def_mode = RoundDown)
                    new_vg_range = vg_ranges[new_vatp]
                    # @assert new_vatp in vatp_range

                    auxlXb = auxXb[new_vatp]
                    for vg in vg_range

                        new_vg = discretize(γ * vg, M.δvg, new_vg_range; def_mode = RoundDown)
                        # @assert new_vg in new_vg_range
