...
function run_simulation!(M::SimModel; 
    ...
    error("Not Implemented; Me (Jose), must fix the discrepancy with SimModel definition (Kg, Kl)")
    ...
    function update_net!(net) 
        ...
        net.ub[vg_idx] = max(net.lb[vg_idx], ub1)

        ub1 = cl * M.D / M.X                # conservation of matter 
        net.ub[vl_idx] = max(net.lb[vl_idx], ub1)
    end
