...
reset!(cont::Container) = (cont.idx = firstindex(cont); cont)
Base.empty!(cont::Container) = (empty!(cont.v); reset!(cont))
...
Base.iterate(cont::Container) = iterate(vec(cont))
Base.iterate(cont::Container, state) = iterate(vec(cont), state)